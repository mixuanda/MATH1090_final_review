...
前三条描述 successor 的 local behavior：successor 不能把两个不同元素合并，
不能把某个元素送回自身，也不能把任何元素送到 \(0\)。Induction 是 global
condition。它可以改写成 subset 语言：若 \(A\subseteq N\) 包含 \(0\)，并且
\(x\in A\Rightarrow S(x)\in A\)，则 \(A=N\)。因此检查 Peano-like examples 时，
要把 local conditions 和 ``从 \(0\) 沿 successor 能否到达全部元素'' 分开处理。
...
\begin{explanation}
判断这类 finite example 时，可以先列出 image table。Injectivity 看 image table
中有没有重复；fixed point 看每个 row 是否满足 \(S(x)\neq x\)；\(0\) not successor
看 \(0\) 是否出现在 image set。Induction 要换一个角度：从 \(0\) 开始反复应用
\(S\)，记录能到达哪些元素。这里 \(0\to1\to2\) 已覆盖整个 set，所以任何满足
base case 和 successor step 的 predicate 都会传到全部元素。真正的问题是 cycle
又从 \(2\) 回到 \(0\)，这违反了 \(0\) 不能有 predecessor 的 axiom。
\end{explanation}
...
问题换成已经早一步出现的 \(a+b\)。所以凡是直接使用这个定义的 induction，通常
应让第二个 variable 变化；若题目让第一个 variable 变化，往往要先建立额外 lemma
把左边的 successor 移到右边或移到整个 expression 外。
...
回溯路径得到不同答案。处理类似题目时，把证明拆成两个问题。Existence 问规则是否
给每个 natural number 一个 value；这依赖每个元素都从 \(0\) 经过有限次 successor
生成。Uniqueness 问两个满足同一 base value 和同一 successor rule 的 functions
能否不同；令 \(P(b)\) 表示两个 values 在 \(b\) 处相同，再用 induction 逐点比较。
这样写能避免把 recursive notation 当作已经存在的 ordinary formula。
\end{explanation}
...
\begin{explanation}
定义只给出 \(a+0=a\)，也就是 zero 在右边的情形。要证明 \(0+n=n\)，固定第一个
input 为 \(0\)，让第二个 input \(n\) 沿 \(0,S(0),S(S(0)),\ldots\) 前进，正好
匹配 \(a+S(n)=S(a+n)\)。类似题目中，先问 recursive definition 是按哪个 input
定义的；induction variable 应尽量选那个 input。
\end{explanation}
...
移动。做类似 ``左边有 \(S(a)\)'' 的题目时，base case 通常由 \(x+0=x\) 直接
收尾；step case 用 recursive definition 把两个 sides 都改写成某个 expression
的 successor，然后套 induction hypothesis。
\end{explanation}
...
联系起来。类似 proof 的常见路线是：先证明右 identity 或左 successor lemma，
再证明 commutativity。不要在 commutativity 还未证明时交换 \(a+b\) 的顺序；每次
换序都必须来自已经证明的 lemma 或 induction hypothesis。
\end{explanation}
...
相同 predecessor。处理含有 \(\exists!\) 的 induction 题时，step case 要分成
existence 和 uniqueness 两段：existence 通常由当前元素本身给出，uniqueness 则
要把任意另一个 candidate 与这个 candidate 比较。
\end{explanation}
...
准备。遇到这种 restriction 时，先列出当前已经允许使用的 facts：\(a+0=a\)、
\(a+S(n)=S(a+n)\)、Peano induction。然后把 proof 写成 base case 加 successor
step，避免借用还在后面才建立的 algebra laws。
\end{explanation}
...
让 \(n\) 出现在右边 input，因此必须用 induction 推动递归定义。做类似等式时，
可以先把两边分别化到同一个 target，例如这里两边都化到 \(S(n)\)。这种 ``共同
target'' 方法比直接在两边来回变形更不容易误用 commutativity。
\end{explanation}
...
逐层加回去。类似计算题不要直接使用熟悉的乘法表；先按 recursive rule 展开到
base case，再用已经定义好的 addition 收缩。这能显示计算确实来自 Peano-style
definition。
\end{explanation}
...
先把这个 lemma 证明出来。写这种长 induction proof 时，最好先标明每个 lemma
负责移动哪一个 successor，避免在 main proof 中同时处理左右 factor、括号和
分配律。
\end{explanation}
...
addition 的 associativity 与 commutativity。类似 distributivity 题目中，先选
能触发 recursive rule 的 variable，再在 step case 中把新增的一份 \(a\) 移到
目标位置；这个移动通常正是 addition laws 发挥作用的地方。
\end{explanation}
...
它。构造 ``只 fail 某一条 axiom'' 的例子时，先决定要破坏哪一种结构：要破坏
injectivity 就制造两个 inputs 有同一个 successor；要破坏 induction 就加入一个
从 \(0\) 到不了的 closed component。之后逐条回查其他 axioms 是否仍然保留。
\end{explanation}
...
由 \(0\) 和 successor operation 生成。证明 induction fail 时，最有效的方法是
给出一个 predicate，而不是只说有额外元素。该 predicate 应包含 \(0\)，对 numeric
successor closed，却排除 disconnected component；这样 hypotheses 成立但
conclusion 失败。
\end{explanation}
...
answer。证明模板是：先选 arbitrary equivalent representatives，写出 relation
给出的等式，再把 operation 的 output 代入同一个 relation。若 output pairs
equivalent，operation 才真正定义在 classes 上，而不是只定义在某些 representatives
上。
...
这里的 relation 避免在 \(\N\) 中直接做 subtraction。等式 \(a-b=c-d\) 被改写成
\(a+d=b+c\)，两边只含 natural-number addition。因此所有 well-definedness proofs
都应该回到 \(\N\) 的 addition、multiplication、cancellation，而不是直接引用尚未
构造好的 integer arithmetic。
...
difference \(a-b\) 的代数直觉在 \(\N^2\) 中的翻译。类似 quotient relation
题目中，reflexive 和 symmetric 往往只是改写定义；transitive 通常需要把两个
relation equations 组合起来，并找出可以 cancellation 的 common factor 或 common
summand。
\end{explanation}
...
必须证明 equivalent pairs 交换后仍 equivalent。解决 similar quotient-definition
problem 时，步骤是：先提出 representative-level formula，再假设
\((a,b)\sim(c,d)\)，最后证明 formula 作用后的 representatives 仍在同一个 class。
\end{explanation}
...
出现。若两个 inputs 都是 quotient classes，通常先只换第一个 representative，
再只换第二个 representative；最后用 transitivity 拼接。这种分步法能把一个复杂
well-definedness proof 拆成两个只含一个 equivalence condition 的计算。
\end{explanation}
...
condition，代数整理更可控。检查答案时要确认最后证明的是 output pairs
equivalent，而不是只证明两个 polynomial expressions 在普通整数直觉下相等；前者
才是 quotient well-definedness 的目标。
\end{explanation}
...
类似 quotient algebra laws 的证明顺序是先 well-defined，再 laws。若顺序反过来，
就会在尚未证明 operation 合法之前使用它。
\end{explanation}
...
Denominator 被限制为 nonzero integer，是因为 cross multiplication 和 cancellation
都会用到这一点。代表元 \((a,b)\) 与 \((-a,-b)\) 可以表示同一个 rational number，
所以任何 relation 或 operation 若直接读取 numerator 的大小或 denominator 的符号，
都必须额外证明它不受换 representative 影响。
...
做 rational quotient 的 transitivity 时，常见策略是把两个 cross-product 等式
乘到同一个中间项，例如这里的 \(bcf\)，再用 nonzero denominator cancellation
去掉中间 denominator。
\end{explanation}
...
products；这正是 relation \(\sim_{\Q}\) 的定义。类似 fraction operation 题目中，
先写出目标 equivalence condition，再把已知的 \(ab'=ba'\)、\(cd'=dc'\) 代入对应
terms；不要只把分子化简到 ``看起来相同''，因为 denominator 已经改变。
\end{explanation}
...
\(\frac cd=\frac{c'}{d'}\) 时，products 必须落在同一个 equivalence class。写类似
证明时，先把需要证明的 relation 展开成一个 equality，再检查已知 equalities
相乘后能否通过 associativity 和 commutativity 重排成目标 equality。
\end{explanation}
...
relation \(\sim_{\Q}\) 只要求 cross products 相同。对 quotient \(\Q\) 的等式，
看到 representatives 不同并不表示结果不同；正确检查是把二者放进
\(\sim_{\Q}\) 的定义，证明交叉相乘后的 products 相等。
\end{explanation}
...
class。类似 inverse problem 中，先确认 proposed denominator nonzero；这里
\(q\neq0\) 的 meaning 是 numerator representative \(a\neq0\)，所以交换后
\((b,a)\) 才是 \(Y\) 中的合法 pair。
\end{explanation}

\section{Relations, gcd, irrationality, and suprema}

这一节的 exercises 都在训练同一个 proof habit：当 objects 是 equivalence classes
或 ordered number systems 时，不能只检查一个 convenient notation。Relations on
\(\Q\) 要对 all representatives 给出同一 truth value；gcd proofs 要比较 common
divisor sets；supremum proofs 要同时证明 upper bound 和 leastness。
...
\(\Q\)。做类似 rational order 题目时，先找一个会改变 denominator signs 的
equivalent representative，例如 \((p,q)\) 换成 \((-p,-q)\)。若 truth value 可能
改变，relation not well-defined；若表达式能化成 positive factor 乘以
\(\frac pq-\frac mn\)，就只依赖 rational numbers 本身。
\end{explanation}
...
\(a=bk+c\)。类似 gcd identity 的证明，最可靠的方法是证明两个 common-divisor
sets 相等：任取一个 divisor，从一个 pair 推到另一个 pair；再反向推回来。最后
因为 positive common divisors 完全相同，greatest element 也相同。
\end{explanation}
...
odd 后，contrapositive 给出需要的结论。类似 divisibility lemma 若正向证明较
绕，可以改证 contrapositive；尤其 parity 问题中，odd 的 normal form
\(2k+1\) 往往比 ``not even'' 更便于计算。
\end{explanation}
...
做类似 \(x^2=N\) 的 rational-solution 题时，先把 \(x\) 写成 lowest terms
\(p/q\)，再把 equation 清 denominators。之后寻找一个 prime 或 parity condition
同时逼迫 \(p\) 和 \(q\) 被同一个 integer 整除。
\end{explanation}
...
就能用 parity argument 逼出 numerator 和 denominator 都 even。类似题目中，
重点不是求 decimal approximation，而是从 lowest-terms equation 中找出一个
divisibility contradiction；一旦同一个 prime 同时整除 numerator 和 denominator，
就与 \(\gcd(p,q)=1\) 冲突。
\end{explanation}
...
时，common divisor 只能是 \(1\)。解决类似问题时，不需要先求出 \(x,y\)；把
\(\gcd(x,y)\) 记为 \(d\)，利用 \(d\mid x\) 和 \(d\mid y\) 推出 \(d\mid\) 给定的
linear combination，通常就能得到答案。
\end{explanation}
...
类似 quadratic rational-solution 题目中，先尝试配方或变量替换，把问题化成
已经知道没有 rational solution 的平方方程，再检查替换是否保持 rationality。
\end{explanation}
...
positive rational gap \(2-u\)。做类似 geometric partial sums 时，先找 closed
form，把 upper-bound claim 变成一个 visible positive tail；leastness 则通过让
tail 小于任意给定 gap 来完成。
\end{explanation}
...
可用的 supremum。类似题目要构造 ``bounded but no rational supremum'' 的 set，
可先在 interval 内放一个 irrational cut，再取 cut 左侧的 rationals。证明时分
\(s<\alpha\)、\(s>\alpha\)、\(s=\alpha\) 三种情况排除 rational supremum。
\end{explanation}
...
类似 decimal-tail problems 中，先写出 infinite repeating decimal 的 rational
value，再比较 finite truncations 与该 value 的 tail；tail 趋向 \(0\) 正是
leastness 的证明工具。
\end{explanation}
...
说明所有后续 terms 都落在更短的 tail 里，于是可以把 upper bound 往下移动。类似
factorial-tail 题目中，denominator-clearing 是核心：选择 \(m\ge q\) 让 \(m!r\)
和 \(m!f(m)\) 都是 integers，再用 tail estimate 制造一个 smaller rational upper
bound。
\end{explanation}
...
\[
  S(x)=x\cup\{x\}.
\]
这个 definition 同时完成两个任务：\(S(x)\) 包含原来的所有 predecessors，也把
\(x\) 本身加入为新的最大 predecessor。因此在这个 model 中，order relation
\(m<n\) 可以由 membership \(m\in n\) 表示。
...
inductive set。我们把 \(\omega\) 作为 \(\N\) 的 Von Neumann model。证明它满足
Peano axioms 时，要分清两种使用方式：Axiom of Infinity 提供至少一个足够大的
inductive set；intersection construction 则保证没有从 \(0\) 和 successor 生成
规则以外的多余元素。
...
遇到类似 model-verification 题目时，应逐条验证 Peano axioms：membership structure
处理 \(0\) not successor、no fixed point、injectivity；minimal inductive set
property 处理 induction。这样 proof 不会把 set-theoretic construction 和 Peano
axioms 的各个要求混在一起。
\end{explanation}
